...
, m_maxConnectedPlayer(0)
, m_connectedPlayers(0)
, m_worldHeight(5000.f)
, m_battleFieldRect(0.f, m_worldHeight - battlefieldSize.y, battlefieldSize.x, battlefielSize.y)
...
{
    for (std::size_t = 0; i < m_connectedPlayers; ++i)
    {
        if (m_peers[i]->ready)
        {
           sf::Packet packet;
...
void GameServer::tick()
{
    ...
    if (now() >= m8timeForNextSpawn + m_lastSpawntime)
    {
        ...
        case Client::PositionUpdate:
            {
                ...
                for (sf::Int32 i = 0; i < numAircrafts; ++i)
                {
                    ...
                    packet >> aircraftIdentifier >> aircraftPosition.x >> aircraftPosition.y >> aircraftHitpoints >> m_missileAmmo;
                    m_aircraftInfo[aircraftIdentifier].position = aircraftPosition;
                    m_aircraftInfo[aircraftIdentifier].hitpoints = aircraftHitpoint;
...
void GameServer::updateClientState()
{
    ...
    FOREACH(auto aircraft, m-AircraftInfo)
...
void GameServer::handleDisconnections()
{
    for (auto itr = m_peers.begin(); itr != m_peers.end();)
    {
        if ((*itr)->timedOut)
        {
            ...
            m_connectedPlayers--;
            m_aircraftCount -= (*itr)->aircraftidentifier.size();
