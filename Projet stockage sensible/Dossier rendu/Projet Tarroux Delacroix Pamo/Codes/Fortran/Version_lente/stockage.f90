...
double precision::A_V, A_S, V_maille,deltat_T,Energie
...
open(UNIT=10,FILE='temperature_stockage.txt',STATUS='UNKNOWN',ACTION='WRITE')
open(UNIT=20,FILE='temperature_finale_stockage.txt',STATUS='UNKNOWN',ACTION='WRITE')

write(20,*),'temps(s)','Temperature(K)'
!**********************************************************************************************************************
!**************************************** CONSTANTES **************************************************************
!**********************************************************************************************************************
...
do boucle_temps=1,Nb_iteration_temps
	call Element(T_1, t, Nb_iteration_espace, Nc, dt, Pression, Ray, L, L_entre_C, debit, Cp_cof, rho_cof, beta,&
				...
				write(20,*),boucle_temps*dt,T_1(Nb_iteration_espace*(Nc+1))
...
double precision::S_l,S_d,S_t,cond_air
integer::NN,k
integer::Pression
...
open(unit=10,FILE='tube.txt',status='old')
read(10,*)Nb_iteration_espace
close(unit=10)
...
open(unit=14,FILE='temperature_pause_destockage.txt',status='old',IOSTAT=ios)

if(ios==0) then
	read(14,*)
	do increment=1,(Nc+1)*Nb_iteration_espace
	read(14,*)T_1(increment)	
	enddo
else
	T_1=T_initial_C
	do k=1, Nb_iteration_espace
...
Write(10,*),'REPARTITION DE LA TEMPERATURE AU SEIN DE L''ECHANGEUR A LA FIN DU STOCKAGE'
