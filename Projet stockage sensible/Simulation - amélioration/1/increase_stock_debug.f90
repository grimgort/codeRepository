...
double precision,dimension(:),allocatable::e, C_S,r
...
!********************* Calcule pour la hauteur maximum *********************
INTEGER(4) IW, errnum
...
!**********************************************************************************************************************
call Geometrie_simple(e, A_V, A_S, C_S, V_maille, Nc, Ray, L, L_entre_C, r)
...
A_1_i_plus_1=-dt/(masse_C*Cp_cof) * ( 2*pi*cond_cof*L/log(r(2)/r(1)) )
A_1_i_i=1+dt/(masse_C*Cp_cof) * ( 0 )
...
do k=2,Nc-1
		A_cof_cst_i_plus_1(k)=2*pi*cond_cof*L/log(r(k+1)/r(k))
		A_cof_cst_i_moins_1(k)=2*pi*cond_cof*L/log(r(k)/r(k-1))
		A_cof_cst_i_i(k)=1+dt/(masse_C*Cp_cof) * ( 0 )
end do

print*,2*pi*cond_cof*L/log(r(k+1)/r(k))
...
DO WHILE(T_1((Nc+1)*Nb_iteration_espace)>T_arret)
	print*,T_arret,T_1((Nc+1)*Nb_iteration_espace), 'ligne(165)'
	!marge d'erruer sur la température acceptable
	if (T_1((Nc+1)*Nb_iteration_espace)-T_arret<marge_erreur_temperature) then
	exit
	print*,'Température de consigne atteinte'
	endif
	...
	if (hauteur>=h_max) then
		CALL cpu_time(t2)
		print*,"temps CPU de l'increase stockage=",t2-t1,'s'
		print*,"ligne(230) La hauteur maximum a été dépassé : Calcule pour h_max puis arret"
		IW = SYSTEM("gfortran -o stockage_Hmax stockage_Hmax.f90")
		IW = SYSTEM("stockage_Hmax")
		Nb_iteration_espace=ceiling(h_max/(2*Ray))
		STOP
	else if (Augmentation_hauteur>=Augmentation_tube_max) then
...
!********************* Variable physique  ************************
double precision,intent(in):: debit, Cp_cof, rho_cof, cond_cof,T_entree, masse_C, cond_air
...
call Convection(h, T_1, Pression, rho_air, Nb_iteration_espace, cond_air,debit,largeur,L)
...
do k=2,Nb_iteration_espace
	i=(k-1)*(Nc+1)+Nc
	A(i,i-1)=-dt/(masse_C*Cp_cof) * ( 0 )
	A(i,i+1)=-dt/(masse_C*Cp_cof) * ( h(i+1)*C_S(Nc) )
	A(i,i)=1+dt/(masse_C*Cp_cof) * ( 0 +h(i+1)*C_S(Nc) )
	B(i)=T_1(i)
end do
...
print*,C_S(Nc),A_S,'ligne(424)'
!********************* Premier élément : dernière maille cofalite  *******************
A(Nc,Nc-1)=-dt/(masse_C*Cp_cof) * ( 0 )
A(Nc,Nc+1)=-dt/(masse_C*Cp_cof) * ( h(Nc+1)*C_S(Nc) )
A(Nc,Nc)=1+dt/(masse_C*Cp_cof) * ( 0 +h(Nc+1)*C_S(Nc) )
B(Nc)=T_1(Nc)
...
do i=1, (Nc+1)*Nb_iteration_espace
	print*, A(i,:)
end do
print*,
...
subroutine geometrie_simple(e, A_V, A_S, C_S, V_maille, Nc, Ray, L, L_entre_C, r)
implicit none

integer,intent(in)::Nc
double precision,intent(in)::Ray,L,L_entre_C
double precision,dimension(:),allocatable,intent(out)::C_S, e, r
double precision,intent(out)::V_maille, A_V, A_S

integer::i
double precision::V_total
double precision,dimension(Nc)::  n
...
subroutine Convection(h, T_1, Pression, rho_air, Nb_iteration_espace, cond_air,debit,largeur,L)
implicit none
...
do k=1,Nb_iteration_espace
	...
	If(S_d<((S_t+2*Ray)/2)) then
		V_max(i)=(S_t/(2*(S_d-2*Ray)))*debit/(largeur*L*rho_air(i))
		! la vitesse max du fluide se situe sur la diagonale entre deux cofalites de ligne différentes 
	else
		V_max(i)=(S_t/(S_t-2*Ray))*debit/(largeur*L*rho_air(i))
