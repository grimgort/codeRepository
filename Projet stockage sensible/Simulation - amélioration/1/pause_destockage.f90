...
open(UNIT=10,FILE='temperature_pause_destockage.txt',STATUS='UNKNOWN',ACTION='WRITE')
...
call CPU_time(t2)
print*,"temps de calcul pour la pause du destockage=",t2-t1,'s'
...
!********************* Cas gÈnÈrale : premiËres mailles cofalite  ************************
...
!********************* Cas gÈnÈrale : mailles centrales cofalite  ************************
do k=1,Nb_iteration_espace
...
!********************* Cas gÈnÈrale : derniËres mailles cofalite  ************************
...
!********************* Premier ÈlÈment : derniËre maille cofalite  ************************
...
!********************* Premier ÈlÈment : maille air  ************************************
A(Nc+1,Nc)=-dt/(masse_A(Nc+1)*Cp_air(Nc+1)) * ( h(Nc+1)*A_S )
A(Nc+1,Nc+1)=1+dt/(masse_A(Nc+1)*Cp_air(Nc+1)) * ( h(Nc+1)*A_S+cond_air*beta*L/(2*Ray) )
...
open(unit=12,FILE='donnees_pause_destockage.txt',status='old')
...
open(unit=13,FILE='temperature_destockage.txt',status='old')
...
Write(10,*),'REPARTITION DE LA TEMPERATURE AU SEIN DE L''ECHANGEUR AVANT LE STOCKAGE'
