...
call variable(Ray, L, L_entre_C, Nc, Nb_iteration_espace, dt, t_max, Pression, T_entree, &
	&T_initial_C, largeur, debit_total, T_arret)
...
T_arret=400
h_max=100		!à rajouter dans le fichier lecture
!***********************************************
...
!*********************  Nb_iteration_espace  ******************************************
Nb_iteration_espace=nint( h_max/(2*Ray) )
...
!**********************************************************************************************************************
!********************* Evaluation initiale du nombre de tube à rajouter  *******************
call Diminution_de_tube_initiale(T_entree, T_arret, Nb_iteration_espace, augmentation_hauteur)

print*,"diminution de la hauteur initiale=",augmentation_hauteur
print*,"Nombre de tube sur la hauteur initiale=",Nb_iteration_espace

if (Nb_iteration_espace<augmentation_hauteur) then
	print*,"La hauteur demandé est trop faible"
	stop
end if
...
!******** initialisationde T pour rentrer dans la boucle *************
...
DO WHILE(T_1((Nc+1)*Nb_iteration_espace)>T_arret)
	...
	call Rajout_de_tube(T_1, T_arret, Nb_iteration_espace, augmentation_hauteur)
enddo
...
CALL cpu_time(t2)
print*,'Temps de calcule',t2-t1
print*,"Pourcentage d'érreur sur la température", (T_1( (Nc+1)*Nb_iteration_espace ) - T_arret )/(T_entree - T_arret)
...
!****************************************************************************************************
!*****************************************  EVALUATION DE LA DIMINUTION DE TUBE ************
!****************************************************************************************************
!****************************************************************************************************
!************************* Cas général *************************
subroutine Rajout_de_tube(T_1, T_arret, Nb_iteration_espace, augmentation_hauteur)
implicit none

double precision,intent(in)::T_arret
integer,intent(in)::Nb_iteration_espace
double precision,dimension(:),intent(in)::T_1
integer,intent(inout)::augmentation_hauteur

if ( T_1( (Nc+1)*Nb_iteration_espace ) - T_arret < 10 ) then
	augmentation_hauteur=5
else if ( T_1( (Nc+1)*Nb_iteration_espace ) - T_arret < 30 ) then
	augmentation_hauteur=20
else if ( T_1( (Nc+1)*Nb_iteration_espace ) - T_arret < 50 ) then
	augmentation_hauteur=40
else if ( T_1( (Nc+1)*Nb_iteration_espace ) - T_arret < 100 ) then
	augmentation_hauteur=70
else
	augmentation_hauteur=100
end if	
end subroutine

!************************* Cas initiale *************************

subroutine Diminution_de_tube_initiale(T_entree, T_arret, Nb_iteration_espace, augmentation_hauteur)
implicit none

double precision,intent(in)::T_arret, T_entree
integer,intent(in)::Nb_iteration_espace
integer,intent(inout)::augmentation_hauteur

if ( T_entree - T_arret < 10 ) then
	augmentation_hauteur=5
else if ( T_entree - T_arret < 30 ) then
	augmentation_hauteur=20
else if ( T_entree - T_arret < 50 ) then
	augmentation_hauteur=40
else if ( T_entree - T_arret < 100 ) then
	augmentation_hauteur=70
else
	augmentation_hauteur=100
end if	
